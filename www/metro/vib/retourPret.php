...
if(isset($_POST["num"]))
{
	...
	foreach($_POST["num"] as $num)
	{
		...
		$str = "SELECT idPret_pret, numInstru_instrument, datePret, '$dateRetour'
		FROM concernepret 
		...
		$str="insert into histo_concernepret values ('$idPret','$numInstru','$datePret','$dateRetour','$nom')";
		$req=mysqli_query($bdd,$str) or die (mysqli_error($bdd));
...
?>
	...
	<div class="container">
		...
		<form action="retourPret.php" method="POST" id="form" onSubmit="return validForm()">
			...
			<div class="page-header">
				<h2>Nom de la personne validant le retour</h2>
			</div>
			<select id="nom" title="Responsable" class="form-control" name="nom" required>
						<option value="" disabled selected>Nom</option>
							<?php
								$str = "SELECT nomEmp, prenomEmp FROM utilisateur WHERE idLabo_Labo = 2;";
