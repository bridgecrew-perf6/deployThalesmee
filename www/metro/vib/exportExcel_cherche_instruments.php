		...
		while($lgMarque=mysqli_fetch_object($Marque)){
			...
			if ($exception == true && strstr($marque,'PCB') ){
				...
				if ($premier == true){
					...
					$sheet->setCellValue('A'.$cptType,$lg->nomDes);
					...
					while($lgmodele=mysqli_fetch_object($modele)){
						...
						$mod = $lgmodele->modele;
						$strCalib="SELECT count(*) as calib FROM `instrument`, localisation, instrument_vib where numInstru_instrument=numInstru and numInstru not in (select `numInstru_instrument` from instrument_vib_capteur) and modele ='$mod' and nomLocal = 'Calibration' and idLocal_Localisation = idLocal;";
						$reqCalib=mysqli_query($bdd, $strCalib);
						$lgCalib=mysqli_fetch_object($reqCalib);
					
						$strCannes="SELECT count(*) as cannes FROM `instrument`, localisation, instrument_vib where numInstru_instrument=numInstru and numInstru not in (select `numInstru_instrument` from instrument_vib_capteur) and modele = '$mod' and nomLocal = 'Prêt Cannes' and idLocal_Localisation = idLocal;";
						$reqCannes=mysqli_query($bdd, $strCannes);
						$lgCannes=mysqli_fetch_object($reqCannes);
					
						$strIntespace="SELECT count(*) as intespace FROM `instrument`, localisation, instrument_vib where numInstru_instrument=numInstru and numInstru not in (select `numInstru_instrument` from instrument_vib_capteur) and modele = '$mod' and nomLocal = 'Prêt Intespace' and idLocal_Localisation = idLocal;";
						$reqIntespace=mysqli_query($bdd, $strIntespace);
						$lgIntespace=mysqli_fetch_object($reqIntespace);
					
						$strMecano="SELECT count(*) as mecano FROM `instrument`, localisation, instrument_vib where numInstru_instrument=numInstru and numInstru not in (select `numInstru_instrument` from instrument_vib_capteur) and modele = '$mod' and nomLocal = 'Prêt Mécano' and idLocal_Localisation = idLocal;";
						$reqMecano=mysqli_query($bdd, $strMecano);
						$lgMecano=mysqli_fetch_object($reqMecano);
					
						$strRebut="SELECT count(*) as rebut FROM `instrument`, localisation, instrument_vib where numInstru_instrument=numInstru and numInstru not in (select `numInstru_instrument` from instrument_vib_capteur) and modele = '$mod' and nomLocal = 'REBUT' and idLocal_Localisation = idLocal;";
				...
				if(!strstr($marque,'PCB') ){
					...
					if ($premier == true){
						...
						$sheet->mergeCells("B".($cptMarque).":B".($cptMarque+$nbLigneMarque-1));
						$cptMarque += $nbLigneMarque;
						$str2 = "SELECT modele, count(*) as total FROM  instrument, instrument_vib where numInstru_instrument=numInstru and marque='$marque' and numInstru not in (select `numInstru_instrument` from instrument_vib_capteur) and `idDes_designation`='$des' group by modele order by modele;";
						if (isset($_GET['DESC'])) {
