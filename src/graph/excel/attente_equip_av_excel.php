...
if(isset($_GET["dateDeb"])) //si on passe des dates en parametres (ex pour le rex) on les prends en comptes
{
	...
	$condVar=array('fifo' => $fifo, 'idService' => $idService);	
...
if ($fifo == 2)	$title = new PHPExcel_Chart_Title('Temps d\'attente tout tests confondus avant le test');  
else $title = new PHPExcel_Chart_Title('Temps d\'attente avant le test (FIFO)');  
