<?php
	...
	$aColumns = array( 'numInstru','nomTypeC','nomDes', 'marque', 'modele', 'numSerie', 'date_futureInt', 'nomLocal');
	
	/* Indexed column (used for fast and accurate table cardinality) */
	$sIndexColumn = "numInstru";
	
	/* DB table to use */
	$sTable = "statut s,instrument_vib_capteur ivc, typeCapteur t,
	instrument i LEFT OUTER JOIN designation de ON i.idDes_designation=de.idDes
	...
	if($sWhere != "")
		...
		$condWhere="where ";
	$condWhere.="i.numInstru=ivc.numInstru_instrument and i.idStatut_statut=s.idStatut and t.idTypeC=ivc.idTypeC_typeCapteur and i.idStatut_statut!=4";
	...
	while ( $aRow = mysqli_fetch_array( $rResult ) )
	{
		...
		for ( $i=0 ; $i<$nbcol ; $i++ )
		{
			if(($i==6) && $aRow[ $aColumns[$i] ]!="")
				$aRow[$i] =date('d/m/Y',strtotime($aRow[ $aColumns[$i] ]));
