<?php
	...
	$dateRef=$date->format('Y-m');
	$date->add(new DateInterval('P1M'));
	$dateRefMoisSuivant=$date->format('Y-m');
	
	$aColumns = array( 'numInstru', 'nomDes', 'nomStatut', 'nomEtat', 'date_derniereInt', 'date_futureInt', 'nomLocal');
	
	/* Indexed column (used for fast and accurate table cardinality) */
	$sIndexColumn = "numInstru";
	
	/* DB table to use */
	$sTable = "statut s, instrument_vth iv, instrument i LEFT OUTER JOIN localisation l ON i.idLocal_localisation=l.idLocal
	...
	if($sWhere != "")
		...
		$condWhere="where ";
	$condWhere.="i.numInstru=iv.numInstru_instrument and i.idStatut_statut=s.idStatut and i.date_futureInt >= '$dateRef-01' and i.date_futureInt < '$dateRefMoisSuivant-01'";
