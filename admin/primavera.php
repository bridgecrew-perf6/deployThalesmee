...
function ajouterEssai($tab,$tabMoyenInconnue,$bdd)
{
	...
	//{
		...
		if($labo!=0)
		{
			...
			$duree_planifie = $tab["target_drtn_hr_cnt"];
			$duree_actuelle = $tab["act_drtn_hr_cnt"];
			...
			$str="SELECT idEssai, max(et.idEtat_etat) as idEtat, date_debut, duree_planifie, date_debut_primavera, duree_planifie_primavera FROM essai e, etatessai et
			...
			if(isset($lg->idEssai) && $lg->idEssai != NULL) //la tache est deja connu
			{
				...
				{
					if ($lg->date_debut_primavera == NULL)
					{
						//champs mis à jours ?
					...
					if($tab["user_field_108024"]!="" && $lg->idEtat ==20)
					{
						...
						if(!$req)
						{
							array_push($essai, "<div class='text-center'><div class='alert alert-warning'><strong>Essai non mis à jours (tache primavera n°$idPrim)</strong></div></div>");
						}
					...
					if ($lg->idEtat >= 22 && $lg->idEtat <= 25 && strtolower($affaire)=="maint")
					{
						...
						/*$str="UPDATE essai set affaire='$affaire', equipement='$equip', os='$os', idMoyen_moyen='$moyen', date_debut='$date_debut', date_fin='$date_fin', date_debut_prevu='$date_debut', date_fin_prevu='$date_fin', date_debut_primavera='$date_debut', duree_planifie = $duree_planifie, duree_actuelle = $duree_actuelle, deux_huit = 0, samedi = 0
						where idTachePrim='$idPrim';";*/
						$str = str_replace("''", "NULL", $str);//on remplace tous les ,'', (du au fait d'une valeur null) par null
						$req=mysqli_query($bdd, $str);
					}else
					{
						array_push($essai, "<div class='text-center'><div class='alert alert-warning'><strong>Essai non ajouté (tache primavera n°$idPrim, affaire : $affaire, of : $of)</br>Identifiant déjà existant</strong></div></div>");
					}
				}
			...
			else //création de l'essai
			{
				...
				{					
					...
					$lg_ligne = mysqli_fetch_object($req);
					$ligne = $lg_ligne->idLigne;
					$str="INSERT into essai values (null, '$idPrim',null,'$affaire','$equip','$os',null,$labo,'$moyen','0','0','1',null,'$date_debut','$date_fin','$date_debut','$date_fin', '$date_debut',0,0,0, '$ligne', 0, $duree_planifie, $duree_actuelle, $duree_planifie, 0, 0);";
				}else
				{					
					$str="INSERT into essai values (null, '$idPrim',null,'$affaire','$equip','$os',null,$labo,'$moyen','0','0','1',null,'$date_debut','$date_fin','$date_debut','$date_fin', '$date_debut',0,0,0, null, 0, $duree_planifie, $duree_actuelle, $duree_planifie,0, 0);";
				}
				...
				if(strtolower($affaire)!="maint")//si ce n'est pas une maintenance
				{
					...
					if(!$req)
					{
						array_push($essai, "<div class='text-center'><div class='alert alert-warning'><strong>Essai non ajouté (tache primavera n°$idPrim)</strong></div></div>");
					}
				...
				else
				{
					...
					if(!$req)
					{
						array_push($essai, "<div class='text-center'><div class='alert alert-warning'><strong>Maintenance non ajouté (tache primavera n°$idPrim)</strong></div></div>");
					}
				}
...
function redirect_post($url, $essai, $moyen_inconnu){
		...
		<body onload="redirect();">
			<form name="redirectpost" method="post" action="<?php echo $url; ?>">
				<?php
					...
					foreach ($essai as $key => $value) {
						echo '<input type="hidden" name=essais[] value="'.htmlspecialchars($value).'">';
					}
					foreach ($moyen_inconnu as $key => $value) {
						echo '<input type="hidden" name=moyens[] value="'.htmlspecialchars($value).'">';
					}
				?>
			</form>
		</body>
...
if(isset($_FILES['file']) && is_uploaded_file($_FILES['file']['tmp_name']))
{
	...
	{	
		...
		if($fic!=false)
		{
			...
			redirect_post ("validation.php", $essai, $tabMoyenInconnue);
