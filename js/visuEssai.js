...
$(document).ready(function() {
	...
	{	
		...
		if (DateDebutPrevu != "null" && test["pastilleOrange"]!=2){
			...
			if (idEtat > 21 && parseInt(tempsRetard.day) < 0 || (idEtat > 21 && parseInt(tempsRetard.day) == 0 && parseInt(tempsRetard.hour) < -2) || (idEtat > 21 && parseInt(tempsRetard.day) == 0 && parseInt(tempsRetard.hour)  == -2 && parseInt(tempsRetard.min) <0 ))
			{
				...
				var url='../essai/majEssaiRetard.php?id='+test["idEssai"];
				$.get( url, function(data) {
				...
				child.innerHTML="<div class='alert alert-danger'><strong>Erreur Ajax </strong></div>";
		...
		if (DateFinPrevu != "null" && test["retardME"]!=2){
			...
			if (idEtat > 22 && parseInt(tempsRetard.day) < 0 || (idEtat > 22 && parseInt(tempsRetard.day) == 0 && parseInt(tempsRetard.hour) < 0) || (idEtat > 22 && parseInt(tempsRetard.day) == 0 && parseInt(tempsRetard.hour)  == 0 && parseInt(tempsRetard.min) <0 ))
			{
				...
				var url='../essai/majEssaiRetard.php?retard=true&id='+test["idEssai"];
				$.get( url, function(data) {
		...
		//si la pastille a été ajouté manuellement
		if (test["pastilleOrange"]==1){
		...
		{
			var url='../essai/majEssaiRetard.php?idRouge='+test["idEssai"];
		...
		if (anomalie == true){
			...
			$("#1"+test["idEssai"]).css("border", "2px solid #FA5858");
		}
	}
	...
	{	
		...
		if (affichage == "semaine"){
			var url='../essai/ajaxEssai.php?date='+ladate.getTime()+'&typeEssai='+typeEssai+'&moyenSelect='+moyenSelect+'&labo='+labo;
			...
			var url='../essai/ajaxEssaiMois.php?date='+ladate.getTime()+'&typeEssai='+typeEssai+'&moyenSelect='+moyenSelect+'&labo='+labo;
		...
		url='../essai/ajaxEssai.php?labo='+labo;
		jQuery.getJSON( url, function(data) {
			//test si le numero entré n'est pas déja dans la liste
			if(data && Object.keys(data).length!=0)
		...
		var url='../essai/trouveNumDate.php?annee='+parseInt(ladate.getFullYear())+'&mois='+parseInt(ladate.getUTCMonth()+1)+'&jour='+parseInt(ladate.getUTCDate()+1);
			$.get( url, function(data) {
	...
	$("#sem_prec").click(function(){
		var url='../essai/focusDate.php?semaine=0';
		$.get( url, function(data) {
	...
	$("#sem_suiv").click(function(){
		var url='../essai/focusDate.php?semaine=1';
		$.get( url, function(data) {
			ladate.setDate(ladate.getDate()+7);
			checkDate();
			creerTab();
	...
	if (affichage == "semaine"){
		...
		$("#jour_suiv").click(function(){
			var url='../essai/focusDate.php?jour=1';
			$.get( url, function(data) {
		...
		$("#jour_prec").click(function(){
			...
			var url='../essai/focusDate.php?jour=0';
			$.get( url, function(data) {
				ladate.setDate(ladate.getDate()-1);
				checkDate();
				creerTab();
	...
	if (affichage == "mois"){
		...
		$("#mois_suiv").click(function(){
			...
			var url='../essai/focusDate.php?mois=1&decalage='+decalage;
			$.get( url, function(data) {
		...
		$("#mois_prec").click(function(){
			...
			var url='../essai/focusDate.php?mois=0&decalage='+decalage;
			$.get( url, function(data) {
